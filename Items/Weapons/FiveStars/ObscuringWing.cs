...
namespace OtakuTech.Items.Weapons.FiveStars
{
    public class ObscuringWing : ModItem
    {
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            Projectile.NewProjectile(position, new Vector2(speedX, speedY), ModContent.ProjectileType<FeatherBlade>(), damage / 5, knockBack, player.whoAmI);
            for (int i = 0; i < numberProjectiles; i++)
            {
                Vector2 perturbedSpeed = new Vector2(speedX, speedY).RotatedBy(MathHelper.Lerp(-rotation, rotation, i / (numberProjectiles - 1))) * item.shootSpeed; // Watch out for dividing by 0 if there is only 1 projectile.
