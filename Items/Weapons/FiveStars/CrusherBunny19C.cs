...
namespace OtakuTech.Items.Weapons.FiveStars
{
    public class CrusherBunny19C : ModItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            if (moddedPlayer.comboCount >= 10)
            {
                ...
                if (moddedPlayer.haxxorDroneCount < moddedPlayer.MAX_haxxorDroneCount)
                {
                    ...
                    int p = Projectile.NewProjectile(player.Center, default, ModContent.ProjectileType<HaxxorDrones>(), item.damage / 2, item.knockBack, Owner: player.whoAmI);
                }
        ...
        //    {
        //        Vector2 perturbedSpeed = new Vector2(speedX, speedY).RotatedByRandom(MathHelper.ToRadians(spread)) * item.shootSpeed; // Watch out for dividing by 0 if there is only 1 projectile.

        //        int p = Projectile.NewProjectile(position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, type, damage / 2, knockBack, player.whoAmI);
